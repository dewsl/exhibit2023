...
int get_all_frames(int timeout_ms, CAN_FRAME can_buffer[], int expected_frames) {
  ...
  do {
      ...
      if (Can0.available()){
        ...
        i++;
        interpret_frame(incoming); //kailangan expected_frames == 
        //if (i == expected_frames){
        process_all_frames(g_can_buffer);
        a = count_frames(g_can_buffer);
        if (a == expected_frames){
          return a;
          break;
          }
        //}
...
void interpret_frame(CAN_FRAME incoming){
  ...
  if ((d1 == 2)|(d1 == 3)|(d1 == 8)|(d1 == 9)|(d1 == 11)|(d1 == 12)|(d1==32)|(d1==33)|(d1 == 41)|(d1 == 42)|(d1 == 51)|(d1 == 52)){ 
...
//    Serial.print(convert_uid_to_gid(id)); Serial.print('\t');
    sprintf(temp, "%5d", x); Serial.print(temp); Serial.print(',');
//    temp[0] = '\0';
    sprintf(temp, "%5d", y); Serial.print(temp); Serial.print(',');
    sprintf(temp, "%5d", z); Serial.print(temp); 
    Serial.print(";");
//    Serial.print(',')
//    temp[0] = '\0';
//    Serial.print(",");
...
//    Serial.println();
  ...
  else if ( (d1==23) | (d1==24)|(d1 == 22)){
    Serial.print(id); Serial.print(',');
    Serial.print(d1); Serial.print(',');
    //Serial.print("  v=");
    int batt = compute_axis(d3,d2); 
    //Serial.print(batt);
    //Serial.print("  temp=");
    int temperature = compute_axis(d5,d4); 
    Serial.print(temperature);//Serial.print(",0,0,0");
    Serial.print("  stat = ");
    Serial.println(d6);
    //Serial.println();
